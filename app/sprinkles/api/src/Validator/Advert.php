...
trait Advert
{
    ...
    function inputCheck(Request $request)
    {
        ...
        $violations[] = $validator->validate($coin_type, array(
            new Assert\NotBlank(array('message' => Translator::translate('%name% not blank', array('%name%' => '货币类型')))),
        ...
        $violations[] = $validator->validate($trade_type, array(
            new Assert\NotBlank(array('message' => Translator::translate('%name% not blank', array('%name%' => '交易类型')))),
            new Assert\Type(array(
                'type' => 'int',
                'message' => '交易类型必须为数字',
            ))
        ...
        $violations[] = $validator->validate($margin, array(
            new Assert\NotBlank(array('message' => Translator::translate('%name% not blank', array('%name%' => '溢价比例')))),
            new Assert\Type(array(
                'type' => 'double',
                'message' => '交易类型必须为数字',
            ))
